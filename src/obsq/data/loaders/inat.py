...
class iNatOccLoader(BaseLoader):
    ...
    async def run(self, pipe:Pipeline, step: PipelineStep):
        ...
        step_name = self.name
        self.queue  = Queue()
        self.table_name = "raw.inat_observations"
        if step.status == StepStatus.completed:
            logger.info(f"{step_name} already completed")
            #SKip 
            return self.table_name
        ...
        # Create table for data
        con.execute(f"CREATE TABLE IF NOT EXISTS  {self.table_name} ")
        last_id = con.execute(f"SELECT MAX(id) FROM {self.table_name}").fetchone()[0] 

        if overwrite and self.table_name in get_all_tables(con):
            if last_id is not None:
                if await _ask_yes_no('Found existing table data on disk, do you want to overwrite all ? (y/n)'):
                    con.execute(f"CREATE OR REPLACE TABLE {self.table_name}")
                    last_id = None
...
class iNatAPI(BaseLoader):
    ...
    def construct_request(self, fields:dict):
        
    #fields_string = f"({fields_to_string(fields)})"
        pass
        
    def _fields_to_string(self,fields_dict:dict, level = 0):
        parts = []
        for key, value in fields_dict.items():
            if isinstance(value, dict):
                nested = self._fields_to_string(value, level + 1)
