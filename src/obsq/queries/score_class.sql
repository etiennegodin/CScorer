...
CASE 
    WHEN (d.month = g.month AND d.distance < 2000 AND cv.cmva_expert_id =1 AND m.meta_media_count >=2 AND m."meta_coordinateUncertaintyInMeters" < 300) THEN 5
    WHEN (d.distance < 5000 AND h.histo_taxon_obs_month_density > 0.5 AND (cv.cmva_expert_id = 1 OR (cv.cmva_id_agree_rate = 1 AND cv.cmva_id_count >=3)) AND m.meta_media_count >=2) THEN 4
    WHEN (r.range = 1 AND o.taxon_id_count > 5 AND cmva_id_count >= 3) THEN 3
    WHEN (r.range =1 AND cv.cmva_id_agree_rate < 1.0 AND h.histo_taxon_obs_month_density < 0.5) THEN 2
    WHEN (r.range = 0 ) THEN 1
    ELSE 0
